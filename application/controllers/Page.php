...
class Page extends CI_Controller {
    ...
    public function index() {
        ...
        if ($category_rows){
            foreach ($category_rows as &$row) {
                ...
                $goods_rows = $this->Inventories_model->get_inventories($row['id']);
                if ($row2s){
        ...
        $brand_rows = $this->Brands_model->get_all_brands();
    ...
    public function productinfo() {
        ...
        $one_category_info = $this->Categories_model->get_by_id($product_data['one_level']);
        $data['category_one'] = $one_category_info;

        if ($product_data['two_level'] != 0){

            $two_category_info = $this->Categories_model->get_by_id($product_data['two_level']);
            $data['category_two'] = $two_category_info;

            if ($product_data['three_level'] == 0) {

                $products = $this->Inventories_model->get_by_level($product_data['one_level'],$product_data['two_level']);
                $data['more_goods'] = $products;
            }

        }else {
            $products = $this->Inventories_model->get_by_level($product_data['one_level']);
            $data['more_goods'] = $products;
        }
        if ($product_data['three_level'] != 0){

            $three_category_info = $this->Categories_model->get_by_id($product_data['three_level']);
            $data['category_three'] = $three_category_info;

            $products = $this->Inventories_model->get_by_level($product_data['one_level'], $product_data['two_level'], $product_data['three_level']);
            $data['more_goods'] = $products;
        }

        $brand_data = $this->Brands_model->get_by_id($product_data['brand']);
        $data['brand'] = $brand_data;

        $product_gallery = $this->Gallery_model->get_galleries($productId);
